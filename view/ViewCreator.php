...
class ViewCreator
{
     ...
     * @param bool $session Indique si on doit ajouter automatiquement 
     *                                           les variables de la session au template.
     */
    function __construct(string $template, array $fields = array(), bool $session = true)
    {
        $this->template = $template;
        $this->fields = $fields;
        if ($session && session_status() === PHP_SESSION_ACTIVE) $this->assignSeveral($_SESSION);
    }
